...
def test_version():
    ...
    subprocess.run([sys.executable, "-m", installer_module, f"--output={deps_dir}", "--traefik-version=1.7.0", "--etcd-version=3.2.25"])
...
def test_linux_platform():
    ...
    subprocess.run([sys.executable, "-m", installer_module, f"--output={deps_dir}", "--platform=linux-amd64"])
...
def test_mac_platform():
    ...
    subprocess.run([sys.executable, "-m", installer_module, f"--output={deps_dir}", "--platform=darwin-amd64"])
